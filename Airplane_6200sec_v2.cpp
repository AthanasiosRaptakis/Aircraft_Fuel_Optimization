...
std::string result_file="RES/Result_6200sec_v2.csv";
std::string param_file="RES/Parameters_6200sec_v2.csv";
// Objective Function Weights 

const double q_CL_d = 0.0001;

const double q_CR_h = 0.01;
...
static void lfcn_cruise_fuel(double *t, double *xd, double *xa, double *u,
  double *p, double *lval, double *rwh, long *iwh, InfoPtr *info) {
	...
	*lval = q_CR_d*(p[1]-p[0])*u[1]*u[1] + q_CR_h*(p[1]-p[0])*(xd[1]-H_p_CR)*(xd[1]-H_p_CR);
