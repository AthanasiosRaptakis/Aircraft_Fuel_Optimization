...
#define  NMOS   4  /* Number of phases (Model Stages) */
...
//Descent HI
#define  NRD_DE_HI_E   4 /* Number of constraints at the end point */
#define  NRDE_DE_HI_E  1 /* Number of equality constraints at the end point */
...
//Interior Points
#define NRD_I   5  /* Number of constraints at interior points */
#define NRDE_I  0  /* Number of equality constraints at interior points */

// Name of .csv file to save Optimisation results 

std::string result_file="RES/Result_Thrust_7km_v2.csv";
std::string param_file="RES/Parameters_Thrust_7km_v2.csv";
...
//************************************************************************
//           Lagrange Objective Function Fuel Consumption - Descent - high
//************************************************************************
static void lfcn_descent_fuel_high(double *t, double *xd, double *xa, double *u,
  double *p, double *lval, double *rwh, long *iwh, InfoPtr *info) {
...
//************************************************************************
//           Lagrange Objective Function Fuel Consumption - Descent - low
//************************************************************************
static void lfcn_descent_fuel_low(double *t, double *xd, double *xa, double *u,
  double *p, double *lval, double *rwh, long *iwh, InfoPtr *info) {

	*lval = q_DE_d*(p[3]-p[2])*u[1]*u[1];
...
static void mfcn_descent_end_time(double *ts, double *xd, double *xa, double *p, double *pr,
  ...
  if (*dpnd) {
      *dpnd = MFCN_DPND(0, *xd, 0, *p, 0);   //*ts, *sd, *sa, *u, *p, *pr
      return;
  }
...
{
	Aircraft_Model_Climb_Thrust(xd, u, rhs, p);
}
//***************************************************
...
static void ffcn_cruise(double *t, double *xd, double *xa, double *u,
  double *p, double *rhs, double *rwh, long *iwh, InfoPtr *info)
{
	Aircraft_Model_Cruise_Thrust(xd, u, rhs, p);
}
//***************************************************
//            Right Hand Side of Descent Stage HI
//***************************************************
static void ffcn_descent_hi(double *t, double *xd, double *xa, double *u,
  double *p, double *rhs, double *rwh, long *iwh, InfoPtr *info)
{
	Aircraft_Model_Descent_Thrust_high(xd, u, rhs, p);
}
//***************************************************
//            Right Hand Side of Descent Stage LO
//***************************************************
static void ffcn_descent_lo(double *t, double *xd, double *xa, double *u,
  double *p, double *rhs, double *rwh, long *iwh, InfoPtr *info)
{
	Aircraft_Model_Descent_Thrust_low(xd, u, rhs, p);
}
//***************************************************
...
//***************************************************
//            Constraints at End point - Descent - HI
//***************************************************
static void rdfcn_descent_hi_e(double *ts, double *sd, double *sa, double *u,
  double *p, double *pr, double *res, long *dpnd, InfoPtr *info)
{
  if (*dpnd) {
    *dpnd = RFCN_DPND(0, *sd, 0, *u, 0, 0);
  ...
  res[0] = sd[1]-H_p_des;
  res[1] = -sd[3];
  res[2] = (Eta(sd[1], sd[2], u[0], sd[4])-Eta_min);
  res[3] = (Eta_max-Eta(sd[1], sd[2], u[0], sd[4]));
}
//***************************************************
...
static void data_out( double *t, double *sd, double *sa, double *u, double *p, double *rwh, long *iwh, InfoPtr *info ) {
	if (*t == 0.) {
		...
		if (t_values.size() > 0) {
			param_stream << p[0] << "\t"<< p[1] << "\t"<< p[2] << "\t"<< p[3] << std::endl;
...
// Define the Descent Phase High
	
		def_mstage(
			2,
			NXD, NXA, NU,
			NULL, lfcn_descent_fuel_high, // mayer term, lagrange term
                        0, 0, 0, NULL, ffcn_descent_hi, NULL,
			NULL, NULL
			);

// Define the Descent Phase Low
	
		def_mstage(
			3,
			NXD, NXA, NU,
			mfcn_descent_end_time, lfcn_descent_fuel_low, // mayer term, lagrange term
                        0, 0, 0, NULL, ffcn_descent_lo, NULL,
			NULL, NULL
			);
...
//********************************************************************
	...
	def_mpc(3, "Start Point", NPR, NRD_DE_HI_E, NRDE_DE_HI_E, rdfcn_descent_hi_e, NULL);
        def_mpc(3, "Interior Point", NPR, NRD_I, NRDE_I, rdfcn_i, NULL);
	def_mpc(3, "End Point", NPR, NRD_DE_E, NRDE_DE_E, rdfcn_descent_e, NULL);
